...
def test_non_optional_precedence():
    ...
    with pytest.raises(NoMatch) as e:
        parser.parse("c")
    assert str(e.value) == ("Expected 'a' or 'b' at position (1, 1) => '*c'.")
...
def test_optional_with_better_match():
    ...
    def grammar():
        return [first, (Optional(second), "six")]
    ...
    assert str(e.value) == ("Expected 'five' at position (1, 20) => 'hree four *5'.")
...
def test_comment_matching_not_reported():
    ...
    with pytest.raises(NoMatch) as e:
        parser.parse("\n\n a // This is a comment \n c")
    assert str(e.value) == ("Expected 'b' at position (4, 2) => 'comment   *c'.")
...
def test_not_match_at_beginning():
    ...
    assert str(e.value) == ("Not expected input at position (1, 1) => '*   one ide'.")
...
def test_not_match_as_alternative():
    ...
    assert str(e.value) == ("Expected 'one' at position (1, 4) => '   *two ident'.")
...
def test_sequence_of_nots():
    ...
    with pytest.raises(NoMatch) as e:
        parser.parse("   two ident")
    assert str(e.value) == ("Not expected input at position (1, 4) => '   *two ident'.")
...
def test_reporting_rule_names_string():
    ...
    with pytest.raises(NoMatch) as e:
        _ = parser.parse("...")
    assert str(e.value) == ("Expected STRING at position (1, 1) => '*...'.")
...
def test_reporting_rule_names_regex():
    ...
    with pytest.raises(NoMatch) as e:
        _ = parser.parse("...")
    assert str(e.value) == ("Expected REGEX at position (1, 1) => '*...'.")

    parser = ParserPython(RegExMatch(r"[^\d\W]\w*\b", rule_name="REGEX", root=False))
    with pytest.raises(NoMatch) as e:
        _ = parser.parse("...")
    assert str(e.value) == ("Expected '[^\\d\\W]\\w*\\b' at position (1, 1) => '*...'.")
...
def test_reporting_rule_names_ordered_choice():
    parser = ParserPython(
        OrderedChoice(["A", "B"], rule_name="ORDERED_CHOICE", root=True)
    )
    with pytest.raises(NoMatch) as e:
        _ = parser.parse("...")
    ...
    parser = ParserPython(
        OrderedChoice(["A", "B"], rule_name="ORDERED_CHOICE", root=False)
    )
    with pytest.raises(NoMatch) as e:
        _ = parser.parse("...")
    assert str(e.value) == ("Expected 'A' or 'B' at position (1, 1) => '*...'.")
